...
```shell script
composer require joyqhs/encryption
```
...
你可以使用 `\Joyqhs\Encryption\Crypt` 类来加密一个值。所有加密过的值都会使用消息认证码 (MAC) 来签名，以检测加密字符串是否被篡改过：
...
你可以使用 `Crypt::decrypt` 方法来进行解密。如果该值不能被正确解密，例如 MAC 无效时，会抛出异常 `Joyqhs\Encryption\Exception\DecryptException`：

```php
use Joyqhs\Encryption\Crypt;
use Joyqhs\Encryption\Exception\DecryptException;
